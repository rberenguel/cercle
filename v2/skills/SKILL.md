...
- `index` — Traverses the codebase (respecting `.gitignore`), parses Go, Python, JS/TS, C++, CSS, HTML, Markdown (`.md`/`.mdx`), and Protobuf (`.proto`), and generates JSON chunk map shards mapped cleanly in `~/.cercle/indexes/`. Re-run this whenever you modify the codebase.
...
7. **Override threshold**: Set `CERCLE_MAX_LINES=<n>` to raise or lower the output threshold for the current invocation (default: 500).
