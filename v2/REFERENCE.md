...
`cercle-lite` is a local, daemonless CLI tool for indexing and searching codebases. It is designed to give LLM agents (and developers) structured access to code without reading whole files into context.

## Installation

```bash
cd v2
go build -o ~/.cercle/cercle-lite .
```
...
Walks the project using `rg --files` (respects `.gitignore`), parses source files with language-specific heuristics, and writes a sharded JSON chunk map to `~/.cercle/indexes/<workspace-hash>/`.
...
Successfully wrote 14 shards to /Users/you/.cercle/indexes/a3f9...
```
...
**Single file:**
```bash
cercle-lite file-skeleton src/auth/login.go
```
```
...
**Directory:**
```bash
cercle-lite file-skeleton src/auth/
```
```
src/auth/login.go:
  12-34: func Login(ctx context.Context, creds Credentials) (Token, error) {
...
```bash
cercle-lite read-chunk src/auth/login.go 25
```
...
```bash
cercle-lite search-lexical "handleLogin|handleLogout"
cercle-lite search-lexical "SELECT.*FROM users"
cercle-lite search-lexical "func.*Error\(\)"
```
...
To search within a specific directory, use a path-anchored regex: `cercle-lite search-lexical "handleLogin" src/auth/` — actually, pass the path as part of the query or use `rg` flags. The current interface only takes a query string; for path filtering run `rg` directly or narrow the query.
...
```bash
cercle-lite search-fuzzy "parseJwtToken"
```
```
...
```bash
cercle-lite find-usages ParseToken
```
```
...
```bash
cercle-lite extract-interface src/auth/token.go
```
...
```bash
cercle-lite search-signature "func.*Handler"
cercle-lite search-signature "func.*\(.*Context"
cercle-lite search-signature "class.*Repository"
```
```
src/http/routes.go:
...
```bash
cercle-lite largest-chunks
cercle-lite largest-chunks 5
```
```
...
```bash
cercle-lite callees src/auth/login.go 15
```
```
...
```bash
cercle-lite dependents src/auth/token.go
```
```
...
cercle-lite read-chunk src/auth/login.go 12

# 5. Search for related code
cercle-lite search-lexical "refreshToken|renewSession"
```
...
The index is written to `~/.cercle/indexes/` (not inside the repo), so re-indexing does not create any tracked files.

### Refactoring: find all callers before renaming a function

```bash
# Find everything that calls OldFunctionName
cercle-lite find-usages OldFunctionName

# For each caller file, get the skeleton so you know the structure
cercle-lite file-skeleton src/payments/

# Read the specific caller
cercle-lite read-chunk src/payments/charge.go 88
```
...
Several commands have a line threshold (default **500 lines**, configurable via `CERCLE_MAX_LINES`). When the output would exceed this threshold, the command returns a **directory-grouped summary** instead of the full content.
...
CERCLE_MAX_LINES=2000 cercle-lite search-lexical "authenticate"

# Lower it for a smaller context budget
CERCLE_MAX_LINES=200 cercle-lite file-skeleton src/
```
...
Indexes are stored globally in `~/.cercle/indexes/<workspace-hash>/` where the hash is the MD5 of the absolute workspace path. Each workspace gets its own directory; multiple projects do not interfere.
...
rm -rf ~/.cercle/indexes/$(echo -n $(pwd) | md5 | cut -c1-32)

# Remove all indexes
rm -rf ~/.cercle/indexes/
```
