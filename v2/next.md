...
`search-lexical`, `file-skeleton` (directory), and `find-usages` now check a line threshold (default 500, overridable via `CERCLE_MAX_LINES`) before returning full content. When the estimated output would exceed the threshold, they return a directory-grouped overview (chunk counts / symbol counts / caller counts) that the agent can use to zoom in.
